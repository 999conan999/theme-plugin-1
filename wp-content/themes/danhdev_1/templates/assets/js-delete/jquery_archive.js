...
function change_play(vl) {
    $('.' + vl).css('background', 'url("/wp-content/themes/danhdev_1/templates/assets/img/play.svg") no-repeat 0 center');
}

function change_pause(vl) {
    $('.' + vl).css('background', 'url("/wp-content/themes/danhdev_1/templates/assets/img/pause.svg") no-repeat 0 center');
}
/** */
...
slideProduct.on('changed.owl.carousel', function(e) {
    ...
    control_show(data[oldIndex], oldIndex);
    if (window.id_audio != undefined) {
        let x = document.getElementById(window.id_audio);
        let icon_play = 'follow-content' + $("#" + window.id_audio).attr('data-z');
        change_play(icon_play);
        if (x.duration > 0 && !x.paused) { x.pause(); }
    }
})
...
function someFunction() {
    jQuery("#show-contact").animate({ height: '241px' });
    $("#text-contact").html(">> Ẩn<br>Liên hệ");
    var is_show = true;
    jQuery("#text-contact").click(function() {
        if (is_show) {
            jQuery("#show-contact").animate({ height: '0px' });
            $(this).html(">> Hiện<br>Liên hệ");
            is_show = false;
        } else {
            jQuery("#show-contact").animate({ height: '241px' });
            $(this).html(">> Ẩn<br>Liên hệ");
            is_show = true;
        }
    });
...
/*cotrol show here*/
function control_show(data, i) {
    if (data != undefined) {
        let result = '<h2 itemprop="headline" id="title1" style="line-height: 1.3; font-size: 24px; display: inline-block; margin: 0; vertical-align: middle; font-weight: 600; color: #000; ">';
        result += data.tieu_de + '</h2>';
        /*gia*/
        if (data.gia != "" && data.gia != undefined) {
            result += '<div style="margin: 12px 0px;"><span style=" font-size: 17px; display: inline-block; margin-right: 18px;">Giá:<ins style="font-size: 24px;color: #d0021b;font-weight: 700;text-decoration: none;width: 140px;display: inline-block;margin-left: 8px;">';
            result += Number(data.gia).format(0, 3, '.') + ' đ</ins></span>';
            result += '<span  onclick=\"myFunction_1(' + data.id + ',\'\',' + data.gia + ',\'' + data.tieu_de + '\',\'' + data.img + '\')"  class="card card-1" style=" padding: 10px 20px; background-color: #e53935; color: white; border-radius: 5px; cursor: pointer;">Mua ngay</span></div>';
        }
        /*table gia*/
        if (data.bang_gia.length > 0 && data.bang_gia != undefined) {
            result += '<table class="card card-1" style=" margin-top: 15px;display:table;" > <thead> <tr> <th>Kích thước</th> <th>Giá</th> <th></th> </tr> </thead> <tbody>';
            data.bang_gia.forEach(e => {
                result += '<tr> <td data-column="Kích thước">' + e.kich_thuoc + '</td>';
                result += '<td data-column="Giá"><ins>' + Number(e.gia).format(0, 3, '.') + ' đ</ins></td>';
                result += '<td data-column=""><button class="mua-btn"  onclick=\"myFunction_1(' + data.id + ',\'' + e.kich_thuoc + '\',' + e.gia + ',\'' + data.tieu_de + '\',\'' + data.img + '\')" >Mua</button></td> </tr>';
            });
            result += '</tbody></table>';
        }
        /*dac diem*/
        if (data.dac_diem != "" && data.dac_diem != undefined) {
            result += '<div class="card card-1 sticker-1" style=" width: 100%; margin-top: 15px; padding: 15px; background: #f3f3f3; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Đặc điểm :</strong>';
            result += '' + data.dac_diem;
            result += '</div>';
        }
        /*uu diem*/
        if (data.uu_diem != "" && data.uu_diem != undefined) {
            result += '<div class="card card-1 sticker-2" style=" width: 100%; margin-top: 15px; padding: 15px; background: #86e473; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Ưu điểm :</strong>';
            result += '' + data.uu_diem;
            result += '</div>';
        }
        /*Nhuoc diem*/
        if (data.nhuoc_diem != "" && data.nhuoc_diem != undefined) {
            result += '<div class="card card-1 sticker-3" style=" width: 100%; margin-top: 15px; padding: 15px; background: #efbdbd; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Nhược điểm :</strong>';
            result += '' + data.nhuoc_diem;
            result += '</div>';
        }
        /*Khuyen mai*/
        if (data.khuyen_mai != "" && data.khuyen_mai != undefined) {
            result += '<div class="card card-1 sticker-6" style=" width: 100%; margin-top: 15px; padding: 15px; background: #feff9d; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Khuyến mãi :</strong>';
            result += '' + data.khuyen_mai;
            result += '</div>';
        }
        /*chinh sach dich vu*/
        if (data.chinh_sach_dich_vu != "" && data.chinh_sach_dich_vu != undefined) {
            result += '<div class="card card-1 sticker-4" style=" width: 100%; margin-top: 15px; padding: 15px; background: #bdcde8; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Chính sách dịch vụ :</strong>';
            result += '' + data.chinh_sach_dich_vu;
            result += '</div>';
        }
        /*Thich hop su dung cho*/
        if (data.thich_hop_su_dung_cho != "" && data.thich_hop_su_dung_cho != undefined) {
            result += '<div class="card card-1 sticker-5" style=" width: 100%; margin-top: 15px; padding: 15px; background: #e0d8d8; " >';
            result += '<strong style=" margin-bottom: 5px; display: block; ">Thích hợp sử dụng cho :</strong><p style="line-height: 1.6;">';
            result += '' + data.thich_hop_su_dung_cho;
            result += '</p></div>';
        }
        $("#show-detail").html(result);
        // console.log(data);
        // console.log(result);
    }
}
...
function myFunction_1(id, size, price, title, img) {
    window.id = id;
    window.size = size;
    window.price = price;
    window.title = title;
    window.img = img;
    window.quantity = 1;
    $("#ppoup").html(show_popup(id, size, price, title, img));
}
/** Hien thi poupup template */
function show_popup(id, size, price, title, img) {
    let show_size = "";
    if (size != "") {
        show_size = '<span style=" margin-bottom: 7px; display: block; "> Kích thước : ' + size + '</span>';
    }
    let popup_temp = '<style>.devvn-popup-content-left { width: 50%; float: left; padding-right: 10px; } .devvn-popup-content-right { width: 50%; float: right; padding-left: 10px; } .devvn-popup-prod { display: table; width: 100%; margin-bottom: 15px; } .devvn-popup-img { display: table-cell; float: none; vertical-align: middle; margin: 0; } .devvn-popup-info { display: table-cell; float: none; overflow: inherit; vertical-align: middle; padding-left: 10px; margin-bottom: 10px; } span.devvn_title { font-size: 18px; color: #333; font-weight: 700; padding-bottom: 10px; display: block; } span.devvn_price { font-size: 16px; color: red; font-weight: 700; padding-bottom: 5px; display: block; } .devvn_prod_variable { margin: 0 0 20px; } .quantity { margin-left: 84px; position: relative; opacity: 1; display: inline-block; display: -ms-inline-flexbox; display: inline-flex; margin-right: 1em; white-space: nowrap; vertical-align: top; } .syx{ font-size: 23px; padding: 4px 15px; cursor: pointer; } .syx:hover{ background-color:#eae1e1; } .devvn-popup-content-right { width: 50%; float: right; padding-left: 10px; } .popup-customer-info-title { font-size: 16px; color: #333; margin-bottom: 15px; font-weight: 700; } .popup-customer-info-group { width: 100%; margin-bottom: 6px; overflow: auto; } .popup-customer-info-item-2 { width: 50%; float: left; padding-right: 3px; } .inp{ font-size: 12px; padding: 2px 5px; border: 1px solid #ccc; width: 100%; height: 26px; box-shadow: none; -moz-box-shadow: none; -webkit-box-shadow: none; margin: 0; font-family: Tahoma,Arial,sans-serif; background: #fff; } .popup-customer-info-item-1 { overflow: hidden; } .popup_quickbuy_shipping_title { padding-right: 10px; display: inline-block; } .popup_quickbuy_total_calc { font-weight: 700; display: inline-block; } .devvn-order-btn { width: 100%; height: 36px; background: #e72b08; color: #fff; font-size: 16px; text-transform: uppercase; border: none; border-radius: 2px; cursor: pointer; outline: none; position: relative; overflow: hidden; margin: 0; padding: 0; } label.error { display: block!important; font-weight: 400; color: red; } .devvn-popup-close:hover { -webkit-transform: rotate( 90deg ); -moz-transform: rotate(90deg); transform: rotate( 90deg ); background-color: transparent; } @media (max-width: 750px){ .devvn-popup-content-left, .devvn-popup-content-right { margin-bottom: 20px; width: 100%; padding-right: 0; padding-left: 0; } }</style>';
    popup_temp += '<div class="dimmer-popup" style=" width: 100%; height: -webkit-fill-available; position: fixed; background-color: #424141; z-index: 100; opacity: 0.6; "></div> <div class="wrap-popup" style="overflow: scroll; max-height:' + (screen.height + 16) + 'px;top: 0px;width: 100%;position: fixed;z-index: 999;padding:10px;"> <div style="width: 100%;max-width: 750px;margin: auto;background-color: white;"> <div style="background: #e72b08;height: 42px; line-height: 42px; padding-left: 20px; padding-right: 40px; font-size: 16px; text-transform: uppercase; color: #fff; position: relative;"> <span style=" display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; ">';
    popup_temp += title + '</span> <button onclick="close_popup()" type="button" class="devvn-popup-close"></button> </div> <div class="content-popup" style="padding: 10px;overflow:auto;"> <div class="devvn-popup-content-left"> <div class="devvn-popup-prod"> <div class="devvn-popup-img"> <img src="';
    popup_temp += img + '" style=" max-width:150px; height: auto; display: inline-block; vertical-align: middle; "> </div> <div class="devvn-popup-info"> <span class="devvn_title">';
    popup_temp += title + '</span>' + show_size + ' <span class="devvn_price">';
    popup_temp += Number(price).format(0, 3, '.') + ' đ</span> </div> </div> <div class="devvn_prod_variable"> <div class="quantity" style=" border: 1px solid #b7b7b7; padding: 2px 2px; "> <span class="syx" onClick="con_tru_sp(-1)">-</span> <span id="quantity-one" style=" font-size: 18px; padding: 3px 1px; ">1</span> <span class="syx" onClick="con_tru_sp(+1)" >+</span> </div> </div> <div class="devvn-popup-content-desc"> Bạn vui lòng nhập đúng số điện thoại để chúng tôi sẽ gọi xác nhận đơn hàng trước khi giao hàng. Xin cảm ơn! </div> </div> <div class="devvn-popup-content-right"> <div class="popup-customer-info-title">Thông tin người mua</div> <div class="popup-customer-info-group"> <div class="popup-customer-info-item-2"> <input id="ten" type="text" class="inp"  placeholder="Họ và tên(bắt buộc)"> </div> <div class="popup-customer-info-item-2"> <input id="sdt" type="number" class="inp" placeholder="Số điện thoại(bắt buộc)"> </div> </div> <div class="popup-customer-info-group"> <div class="popup-customer-info-item-1"> <textarea id="dia_chi" class="inp" placeholder="Địa chỉ(bắt buộc)" style=" height: 58px; min-height: inherit; "></textarea> </div> </div> <div class="popup-customer-info-group"> <div class="popup-customer-info-item-1"> <textarea id="ghi_chu" class="inp" placeholder="Ghi chú đơn hàng" style=" height: 58px; min-height: inherit; "></textarea> </div> </div> <div class="popup-customer-info-group"> <div class="popup-customer-info-item-1"> <div class="popup_quickbuy_shipping_title">Tổng:</div> <div class="popup_quickbuy_total_calc">';
    popup_temp += Number(price).format(0, 3, '.') + ' đ</div> </div> </div> <div class="popup-customer-info-group"> <div class="popup-customer-info-item-1"> <button onclick="click_mua_ngay()" type="button" class="devvn-order-btn">Đặt hàng ngay</button> </div> </div> <div class="popup-customer-info-group"> <div id="error-show" class="popup-customer-info-item-1"> </div> </div> </div> </div> </div> </div>';
    return popup_temp;
}
/**loading template */
function loading() {
    return '<style> .loader { position: absolute; top: 45%; left: 50%;border: 6px solid #f3f3f3; border-radius: 50%; border-top: 6px solid #d81717; width: 40px; height: 40px; -webkit-animation: spin 2s linear infinite; /* Safari */ animation: spin 2s linear infinite; } @-webkit-keyframes spin { 0% { -webkit-transform: rotate(0deg); } 100% { -webkit-transform: rotate(360deg); } } @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } </style> <div class="dimmer-popup" style="width: 100%; height: -webkit-fill-available; position: fixed; background-color: #424141; z-index: 100; opacity: 0.6; "> <div class="loader"></div> </div>';
}
/** popup dat hang thanh con */
function dat_hang_thanh_cong_template(id, size, price, title, quantity, ten, dia_chi, sdt, ghi_chu, url) {
    let show_size = "";
    if (size != "") {
        show_size = '<span style=" margin-bottom: 7px; display: block; "> Kích thước : ' + size + '</span>';
    }
    template_pop = '<style> .wrap-pp { margin: auto; max-width: 800px; margin-top: 8%; box-shadow: 0 1px 3px rgb(0 0 0 / 12%), 0 1px 2px rgb(0 0 0 / 24%); transition: all .3s cubic-bezier(.25,.8,.25,1); -webkit-border-radius: 2px; -moz-border-radius: 2px; border-radius: 2px; background: #fff; padding: 25px 25px 15px; text-align: center; border-top: 10px solid #33a938; } .check { margin: auto; padding: 20px; width: 109px; border-radius: 50%; background-color: #33a938; -webkit-animation: fall-in .75s; -moz-animation: fall-in .75s; -o-animation: fall-in .75s; animation: fall-in .75s; box-shadow: 0 1px 3px rgb(0 0 0 / 12%), 0 1px 2px rgb(0 0 0 / 24%); margin-top: -75px; } </style> <div style=" overflow-y: scroll;position: fixed; width: 100%; height: 100%; background-color: #0e0e0ed6; top: 0px; left: 0px; z-index: 99;"> <div class="container"> <div class="row" style=" margin: 40px -8px; "> <div class="wrap-pp col-sm-8 col-md-6 col-lg-7"> <div class="header-pp"> <div class="check"><span class="icon-check" style="margin: auto;font-size: 70px;color: white;"></span></div> <div> <p style=" font-size: 22px; margin-top: 12px; font-weight: 600; margin-left: 12px; ">Đặt Hàng Thành Công!</p> </div> </div> <div class="body-pp"> <p style=" text-align: left; margin-bottom: 8px; color: currentColor;margin-top: 12px; ">Thông tin đơn hàng :</p> <table style=" border-collapse: collapse; width: 100%; "> <thead> <tr style=" background-color: burlywood; "> <th style="background-color: burlywood;color: black;font-size: 14px;">Tên sản phẩm</th> <th style="background-color: burlywood;color: black;font-size: 14px;">Giá thành</th> <th style="background-color: burlywood;color: black;font-size: 14px;">Số lượng</th> <th style="background-color: burlywood;color: black;font-size: 14px;">Thành tiền</th> </tr> </thead> <tbody> <tr> <td data-column="Tên sản phẩm" style="background-color: white;" ><a style="color: #333;font-size: 14px;background-color: white;" >';
    template_pop += title + show_size + '</a></td> <td data-column="Giá" style="color: #333;font-size: 14px;background-color: white;">';
    template_pop += Number(price).format(0, 3, '.') + ' đ</td> <td data-column="Số lượng" style="color: #333;font-size: 14px;background-color: white;">';
    template_pop += quantity + '</td> <td data-column="Thành tiền"style="color: #333;font-size: 14px;background-color: white;">';
    template_pop += Number(price * quantity).format(0, 3, '.') + ' đ</td> </tr> <tr> <td></td> <td></td> <td style=" font-weight: 600;font-size: 14px; ">Tổng tiền: </td> <td style="font-weight: 600; color: blue;font-size: 14px;">';
    template_pop += Number(price * quantity).format(0, 3, '.') + ' đ</td> </tr> </tbody> </table> <div> <p style=" text-align: left; margin-bottom: 8px; color: currentColor;margin-top: 6px;">Thông tin người nhận :</p> <p style="text-align: left;margin-bottom: 8px;color: currentColor;margin-top: 6px;">Tên : <b>';
    template_pop += ten + '</b></p> <p style="text-align: left;margin-bottom: 8px;color: currentColor;margin-top: 6px;">Địa chỉ : <b>';
    template_pop += dia_chi + '</b></p> <p style="text-align: left;margin-bottom: 8px;color: currentColor;margin-top: 6px;">Số điện thoại : <b>';
    template_pop += sdt + '</b></p> <p style="text-align: left;margin-bottom: 8px;color: currentColor;margin-top: 6px;">Ghi chú :';
    template_pop += ghi_chu + '</p><div class="footer-pp"> <a href="';
    template_pop += url + '"> <p class="bnt-home" style=" margin: auto; width: 162px; padding: 14px; border: 1px solid green; border-radius: 10px; font-size: 15px; font-weight: 600; color: green; ">Quay lại trang chủ</p> </a> </div> </div> </div> </div> </div>';
    return template_pop;
}
/**close popup */
...
function show_click_quantity() {
    $("#quantity-one").html(window.quantity);
    $(".popup_quickbuy_total_calc").html(Number(window.price * window.quantity).format(0, 3, '.') + ' đ');
}

/**function dat hang */
function click_mua_ngay() {
    ...
    dia_chi = $("#dia_chi").val();
    ghi_chu = $("#ghi_chu").val();
    $("#ten").css("border-color", "#bbb2b2");
    $("#sdt").css("border-color", "#bbb2b2");
    $("#dia_chi").css("border-color", "#bbb2b2");
    $("#error-show").html("")
    if (ten == "") {
        is_ok = false;
        array_red.push("ten");
        mess += '<label class="error" for="ten">*Họ và tên là bắt buộc</label>';
    }
    if (sdt == "") {
        is_ok = false;
        array_red.push("sdt");
        mess += '<label class="error" for="sdt">*Số điện thoại là bắt buộc</label>';
    } else {
        if (vnf_regex.test(sdt) == false) {
            is_ok = false;
            array_red.push("sdt");
            mess += '<label class="error" for="sdt">*Số điện thoại bạn điền không đúng</label>';
        }
    }
    if (dia_chi == "") {
        is_ok = false;
        array_red.push("dia_chi");
        mess += '<label class="error" for="dia_chi">*Bạn điền thiếu địa chỉ kìa</label>';
    } else {
        if (dia_chi.length < 8) {
            is_ok = false;
            array_red.push("dia_chi");
            mess += '<label class="error" for="dia_chi">*Địa chỉ bạn ghi chi tiết hơn nữa, giúp bên mình giao qua nhanh hơn đó</label>';
        }
    }
    if (!is_ok) {
        array_red.forEach(e => {
        ...
        $("#error-show").html(mess)
    } else {
        let home_url = $("#ppoup").attr("data-src");
        let url = home_url + '/wp-content/themes/danhdev_1/templates/ajax/order.php';
        $.ajax({
            type: 'POST',
            url: url,
            data: {
                ...
                ghi_chu: ghi_chu,
                ten_sp: window.title,
                kich_thuoc: window.size,
                so_luong: window.quantity,
                url: home_url + '/?p=' + window.id,
                gia: window.price
            ...
            success: function(data) {
                $("#ppoup").html(dat_hang_thanh_cong_template(window.id, window.size, window.price, window.title, window.quantity, ten, dia_chi, sdt, ghi_chu, home_url + '/'));
            }
        });
    }

}
$(document).bind('contextmenu', function (e) { return false; });$('body').bind('cut copy paste', function (e) { return false;});